...
class BirthdayListView(BirthdayMixin, ListView):
    # По умолчанию этот класс  выполняет запрос queryset = Birthday.objects.all(),
    # но мы его переопределим:
    # А в конце допишем запрос в атрибуте queryset класса BirthdayListView: 
    # прямо в него добавим запрос к объектам, связанным с Birthday через поле author.
